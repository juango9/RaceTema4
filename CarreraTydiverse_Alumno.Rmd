...
```{r}
cran %>% filter(country == 'ES')%>%filter(version > '3.4.0')
```
...
```{r}
cran3<-cran3%>%mutate(size_gb=size/1024)
```
...
```{r}
cran3<-cran3%>%mutate(size=size-1000)
```
...
```{r}
by_package %>% summarize(mediasize = mean(size))
```
